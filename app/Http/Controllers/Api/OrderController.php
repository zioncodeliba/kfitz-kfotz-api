...
class OrderController extends Controller
{
    ...
    public function update(Request $request, string $id)
    {
        ...
        if ($previousStatus !== Order::STATUS_DELIVERED && $order->status === Order::STATUS_DELIVERED) {
            $this->notifyOrderEvent($order, 'order.delivered');
        }
    ...
    public function updateShippingSettings(Request $request, string $id)
    {
        ...
        $carrier = null;
        $existingShipment = $order->shipment()->latest('created_at')->first();
        $previousTrackingNumber = $existingShipment?->tracking_number ?? $order->tracking_number;
        ...
        if ($shipment) {
            ...
            $currentTrackingNumber = $shipment->tracking_number ?? $order->tracking_number;
            if ($shippingTypeValue !== 'pickup'
                && !empty($currentTrackingNumber)
                && ($previousTrackingNumber === null || $previousTrackingNumber === '' || $previousTrackingNumber !== $currentTrackingNumber)
            ) {
                $this->notifyOrderEvent($order, 'order.shipment_registered');
            }
        }
    ...
    {
        return $this->orderEmailPayloadService->build($order);
    }
